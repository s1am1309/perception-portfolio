...
/* =========================================================
   GRADIENT COLOR PALETTE - Enhanced with atmospheric colors
========================================================= */
...
/* =========================================================
   SMOOTH EASING — enhanced with cinematic curves
========================================================= */
...
/* =========================================================
   DEVICE DETECTION HOOK - Enhanced
========================================================= */
...
function useDeviceDetect() {
  ...
  return { isMobile, isTablet, deviceType, isTouch };
}

/* =========================================================
   SCENE LIFECYCLE — enhanced with bloom effects
========================================================= */
...
function Scene({
  ...
  useFrame(() => {
    ...
    const isAhead = cameraZ > sceneZ;
    let visibility = 0;

    if (isAhead) {
      if (distance < ACTIVATION_DISTANCE) {
        if (distance > FULL_DISTANCE) {
          visibility = smoothstep(ACTIVATION_DISTANCE, FULL_DISTANCE, distance);
        } else if (distance > NEAR_DISTANCE) {
          visibility = 1;
        } else {
          visibility = smoothstep(0, NEAR_DISTANCE, distance);
        }
      }
    }
    ...
    if (visible !== previousActive.current) {
      previousActive.current = visible;
    ...
    // Glow intensity smooth
    const targetGlow = currentVisibility;
    const glowSmoothing = isMobile ? 0.1 : 0.06;
    smoothGlow.current += (targetGlow - smoothGlow.current) * glowSmoothing;

    // Apply smooth opacity and glow to all meshes
    group.current.traverse((child) => {
      ...
      materials.forEach((mat) => {
        ...
        if (mat instanceof THREE.MeshBasicMaterial) {
          ...
          const glowIntensity = smoothGlow.current * 0.15;
          mat.color.setRGB(
            baseColor.r + glowIntensity,
            baseColor.g + glowIntensity * 0.8,
            baseColor.b + glowIntensity * 1.2
          );
        }
      });
    });
...
/* =========================================================
   CAMERA — enhanced with subtle atmosphere changes
========================================================= */
...
function CameraController() {
  ...
  const previousScrollSpeed = useRef(0);
  const { isMobile } = useDeviceDetect();
  const smoothFogDensity = useRef(0.015);
  const smoothFogColor = useRef(new THREE.Color(COLORS.background));

  useFrame((state) => {
    const progress = getScrollProgress();
    const scrollSpeed = Math.abs(progress - (previousScrollSpeed.current || 0)) * 100;
    previousScrollSpeed.current = progress;

    targetZ.current = CAMERA_START_Z - progress * (CAMERA_START_Z - CAMERA_END_Z);

    // Enhanced smooth camera with dynamic damping
    const damping = isMobile ? 0.06 : 0.035 + (1 - progress) * 0.02;
    currentZ.current += (targetZ.current - currentZ.current) * damping;
    ...
    // Atmospheric fog with smooth transitions
    const targetFogDensity = isMobile ? 0.015 + progress * 0.03 : 0.02 + progress * 0.04;
    smoothFogDensity.current += (targetFogDensity - smoothFogDensity.current) * 0.05;
    
    // Subtle color shift based on progress
    const targetFogColor = new THREE.Color(COLORS.background);
    if (progress > 0.3 && progress < 0.6) {
      targetFogColor.lerp(new THREE.Color(COLORS.indigo), 0.1);
    } else if (progress > 0.6) {
      targetFogColor.lerp(new THREE.Color(COLORS.violet), 0.15);
    }
    smoothFogColor.current.lerp(targetFogColor, 0.03);
    
    scene.fog = new THREE.FogExp2(
      smoothFogColor.current,
      smoothFogDensity.current
    );

    // Subtle camera shake during fast scroll
    if (scrollSpeed > 5 && !isMobile) {
      const shakeIntensity = Math.min(scrollSpeed * 0.0005, 0.02);
...
/* =========================================================
   HERO — enhanced with particle field and light rays
========================================================= */
...
function Hero() {
  ...
  const smoothRingRotation = useRef(0);
  ...
  useFrame((state) => {
    ...
    const targetRotX = mesh.current.rotation.x + speed;
    const targetRotY = mesh.current.rotation.y + speed * 2;
    
    smoothRotationX.current += (targetRotX - smoothRotationX.current) * 0.1;
    smoothRotationY.current += (targetRotY - smoothRotationY.current) * 0.1;
    
    mesh.current.rotation.x = smoothRotationX.current;
    mesh.current.rotation.y = smoothRotationY.current;

    // Ring rotation
    if (ringRef.current) {
      smoothRingRotation.current += (0.0008 - smoothRingRotation.current) * 0.1;
      ringRef.current.rotation.z = smoothRingRotation.current;
    }
  });
  ...
  return (
    <Scene scene={JOURNEY.hero}>
      <group
        position={[0, 0, JOURNEY.hero.z]}
        scale={mobile ? 0.65 : 0.85}
      >
        {/* Main icosahedron */}
        <mesh ref={mesh}>
          <icosahedronGeometry args={[mobile ? 3.5 : 5, mobile ? 3 : 5]} />
          <meshBasicMaterial
        ...
        <mesh ref={ringRef} position={[0, 0, -0.1]}>
          <torusGeometry args={[mobile ? 4.5 : 6.5, mobile ? 0.02 : 0.03, 16, 64]} />
          <meshBasicMaterial
        ...
        <mesh position={[0, 0, -0.2]}>
          <torusGeometry args={[mobile ? 5.5 : 7.5, mobile ? 0.01 : 0.015, 16, 64]} />
          <meshBasicMaterial
            color={COLORS.indigo}
            wireframe
            transparent
...
/* =========================================================
   ABOUT — unchanged content with enhanced visuals
========================================================= */
...
function About() {
  ...
  return (
    <Scene scene={JOURNEY.about}>
      <group
        position={[0, 0, JOURNEY.about.z]}
        scale={isMobile ? 0.65 : 0.85}
      >
        <Text
          position={[0, isMobile ? 4.0 : 4.8, 0]}
...
/* =========================================================
   PROJECT OBJECT — enhanced with glow and particles
========================================================= */
...
function ProjectObject({
  ...
  useFrame((state) => {
    ...
    smoothHover.current += (targetHover - smoothHover.current) * 0.15;
    
    if (glowMesh.current) {
      glowMesh.current.scale.setScalar(1 + smoothHover.current * 0.15);
      glowMesh.current.position.y = smoothY.current;
    }
  });
...
function Projects() {
  ...
  return (
    <Scene scene={JOURNEY.projects} onActiveChange={setActive}>
      <group
        position={[0, 0, JOURNEY.projects.z]}
        scale={isMobile ? 0.65 : 0.85}
      >
        <Text
...
function Contact() {
  ...
  return (
    <Scene scene={JOURNEY.contact}>
      <group
        position={[0, 0, JOURNEY.contact.z]}
        scale={isMobile ? 0.65 : 0.85}
      >
        <Text
...
/* =========================================================
   PARTICLES — enhanced with color variation
========================================================= */
...
/* =========================================================
   AURORA BACKGROUND — new atmospheric effect
========================================================= */
...
function AuroraBackground() {
  ...
  useFrame((state) => {
    ...
    // Smooth pulsing aurora
    const targetOpacity = 0.15 + Math.sin(state.clock.elapsedTime * 0.3) * 0.05;
    smoothOpacity.current += (targetOpacity - smoothOpacity.current) * 0.05;
    
    const material = mesh.current.material as THREE.MeshBasicMaterial;
    material.opacity = isMobile ? smoothOpacity.current * 0.7 : smoothOpacity.current;
  });

  return (
    <mesh ref={mesh} position={[0, 0, -30]} scale={[30, 20, 1]}>
      <planeGeometry />
      <meshBasicMaterial
...
/* =========================================================
   WORLD — enhanced with atmospheric layers
========================================================= */
...
export default function World() {
  ...
  return (
    <div
      ...
      style={{
        background: "radial-gradient(circle at 50% 50%, #100B24 0%, #05030D 45%, #020204 100%)",
      }}
    >
      <Canvas
        camera={{
